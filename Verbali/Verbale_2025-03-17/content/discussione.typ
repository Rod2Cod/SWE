...
#decisioni(
 [
   ...
   2. Ulteriore divisione delle singole interfacce (\#176)
 ],
...
== Discussione progettazione
 ...
 [
   ...
   2. All'aggiornamento di un elemento domanda, se è all'interno di un risultato test, la versione vecchia viene duplicata -> la versione vecchia viene mantenuta con un id diverso, la versione nuova mantiene l'Id originale
 ],
 [
   1. La decisione è stata presa per evitare che il caricamento di un file possa portare a errori di coerenza dei dati
   2. La decisione è stata presa per evitare che la modifica di un elemento domanda sia incoerente con i risultati dei test già fatti su quell'elemento domanda
 ]
 )
